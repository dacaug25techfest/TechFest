...
public class AdminController {
    ...
    // View all users (for organizer verification & platform activity)
    @GetMapping("/users")
    public ResponseEntity<List<User>> viewAllUsers() {
        return ResponseEntity.ok(adminService.getAllUsers());
    }

    // View users by role (e.g. organisers)
    @GetMapping("/users/role/{roleId}")
    public ResponseEntity<List<User>> viewUsersByRole(@PathVariable Long roleId) {
        return ResponseEntity.ok(adminService.getUsersByRole(roleId));
    }

    // Approve organizer
    @PutMapping("/users/{uid}/approve")
    public ResponseEntity<User> approveUser(@PathVariable Long uid) {
        return ResponseEntity.ok(adminService.approveUser(uid));
    }
}
